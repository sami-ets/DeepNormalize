...
class ModelHelper(object):
    ...
    @staticmethod
    def assert_model_params_changed(initial_params, params):
    ...
    @staticmethod
    def assert_model_params_not_changed(initial_params, params):
        for (_, p0), (name, p1) in zip(initial_params, params):
            try:
    ...
    @staticmethod
    def assert_model_grads_are_none(params):
        for (name, p) in params:
            try:
                assert p.grad is None or p.grad.sum() == 0
            except AssertionError:
                raise ValueError(" {var_name} {msg}".format(var_name=name, msg="is not None."))

    @staticmethod
    def assert_model_grads_are_not_none(params):
        for (name, p) in params:
            try:
                assert p.grad is not None or p.grad.sum() != 0
            except AssertionError:
                raise ValueError(" {var_name} {msg}".format(var_name=name, msg="is None."))
